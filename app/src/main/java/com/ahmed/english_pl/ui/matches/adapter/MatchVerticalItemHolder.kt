...
class MatchVerticalItemHolder(
    private val binding: ItemMatchVerticalBinding,
    ...
    private fun bindMatchScore(score: Score?, status: String?, utcDate: String?) {
        if (status == "SCHEDULED") {
            ...
            binding.txtMatchPenalties.isVisible = false
            return
        }
        ...
        if (score?.penalties?.homeTeam != null && score.penalties.awayTeam != null) {
            binding.txtMatchPenalties.isVisible = true
            binding.txtMatchPenalties.text =
                "Pt: ${score?.penalties.homeTeam} - ${score?.penalties.awayTeam}"
        } else {
            binding.txtMatchPenalties.isVisible = false
        }
    }
